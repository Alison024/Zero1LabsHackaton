...
import { GatewayContract } from "../typechain-types";
...
const ifaceResultCallback = new ethers.Interface([
...
const getAlreadyFulfilledDecryptions = async (): Promise<[bigint]> => {
  ...
  results = results.concat(
    pastResults.map((result) => ifaceResultCallback.parseLog(result).args[0])
  );
    ...
    if (!results.includes(requestID)) {
      ...
      if (mocked) {
        ...
        const isAllowedForDec = await Promise.all(
          handles.map(async (handle) => acl.allowedForDecryption(handle))
        );
        ...
        const tx = await gateway
          .connect(relayer)
          .fulfillRequest(requestID, calldata, [], { value: msgValue });
        await tx.wait();
      } else {
